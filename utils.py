...
def a_inv_times_b(
    ...
    """
    try:
        # Se siamo su MPS e i dati sono complessi, Apple non supporta linalg.solve.
        # Spostiamo su CPU, risolviamo e torniamo su GPU.
        if a.device.type == 'mps' and a.is_complex():
            a_cpu = a.detach().cpu()
            b_cpu = b.detach().cpu()
            c_cpu = torch.linalg.solve(a_cpu, b_cpu)
        ...
        # Gestione errore MPS (se non intercettato dall'if sopra)
        if 'MPS only supports floats' in str(e) or 'linalg.lu_factor' in str(e):
            a_cpu = a.detach().cpu()
            b_cpu = b.detach().cpu()
            c_cpu = torch.linalg.solve(a_cpu, b_cpu)
            return c_cpu.to(a.device)
            
        # Gestione del tuo errore originale sulle dimensioni
